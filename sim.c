    ...
    char **argv;            /* Argument strings. */
{
    ...
    if(machPtr->strategy==1){
        machPtr->penalties+= machPtr->branchYes + machPtr->branchNo;
        printf("%d", machPtr->penalties);
    } else if(machPtr->strategy==2){
        machPtr->penalties+= machPtr->branchYes;
    }
    ...
    printf("Total cycles = %d\n", intCount + floatCount + 
           machPtr->loadStalls + machPtr->penalties + machPtr->FPstalls);
    interp->result[0] = '\0';
    }
                     ...
                     * breaks found. */
{
    ...
    switch (wordPtr->opCode) {
        ...
        machPtr->branchYes++;
          ...
          if(machPtr->strategy>0)
          	machPtr->operationCount[OP_NOP]--;
          break;
